 		...
 		function renderList(data) {
			var searchname = $o.val().charAt(0).toUpperCase() + $o.val().substr(1);
 			$.each(data, function(index) {
 				...
 				if (this.address && (this.address.city == searchname
 									|| this.address.town == searchname  
 									|| this.address.hamlet == searchname 
 									|| this.address.village == searchname) 
 					) {
 					try {
 						//var title = (this.address.city || this.address.town || this.address.hamlet || this.address.village) + ', ' + (this.address.county?(this.address.county+', '):'') + (this.address.state?(this.address.state+', '):'') + this.address.country;
						var title = (this.address.city || this.address.town || this.address.hamlet || this.address.village);
 						var arr = [];
						if (this.address.country && typeof this.address.country !=='undefined') arr.push(this.address.country);
						if (this.address.state && typeof this.address.state !== 'undefined') arr.push(this.address.state);
						if (this.address.state && typeof this.address.county !== 'undefined') arr.push(this.address.county);						
						$('<a href="'+opt.goto+'/?fi='+this.lat+'&la='+this.lon+'&title='+encodeURI(title)+'"><span style="font-weight: 600">'+(this.address.city || this.address.town || this.address.hamlet || this.address.village)+'</span></a> <span style="color: #666;">'+ arr.join(', ') + '</span>').appendTo($div);
						delete arr;
 			...
 			if (typeof data == 'object')
	 			try {
	 				...
	 				if (data.address && (data.address.city || data.address.town || data.address.hamlet || data.address.village)) {
	 					var title2 = (data.address.city || data.address.town || data.address.hamlet || data.address.village) + ', ' + data.address.country + (data.address.state?(', '+ data.address.state):'');
						...
						title = title.replace('сельское поселение ', '');						
	 					$('<span>Ближайший населённый пункт:</span> <a href="' + opt.goto + '/?fi=' + data.lat + '&la=' + data.lon + '&title='+encodeURI(title)+'"><span>' + title2 + '</span></a>').appendTo($div);
	 				}
