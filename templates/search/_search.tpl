  	...
  	function add($sql, $template, $title) {
		...
		if ($sth->rowCount()>0) {
			...
			while ($row = $sth->fetch()) {
				...
				try {
					if ($title === "Города")
						$this->pushString.= '<div '.($i<$rowCount-1?'style="margin-bottom: 7px;"':'').'>'.iconv("UTF-8", "windows-1251", str_replace('%class', $id, str_replace('%name', $row['name'], str_replace('%href', str_replace(" ", "-", strtolower($row['id'].(isset($row['name_en'])?('-'.str_replace(" ", "_", $row['name_en'])):""))), $template_))).'</div>');
				if ($title === "Аэропорты")
...
<meta name="keywords" content="погода прогноз голос робот погода <%= country_name %> карта магнитные бури две недели Поиск  по "<?php echo iconv("UTF-8", "windows-1251", $search->pathTokens[0])?>"">
