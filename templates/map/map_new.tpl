...
<script type="text/javascript">
	...
	var mapTypes = {
		...
		"TMIN_2_m_above_ground": {"steps": {"set": [1,3,6,12], "def": 6}, "name": "Минимальная температура", "descr": "Минимальная температура воздуха в приземном слое за последние %hours% перед сроком составления карты"},
		"TMAX_2_m_above_ground": {"steps": {"set": [1,3,6,12], "def": 6}, "name": "Максимальная температура", "descr": "Максимальная температура воздуха в приземном слое за последние %hours% перед сроком составления карты"},
		"TMP_surface": {"steps": {"set": [1,3,6,12], "def": 6}, "name": "Температура поверхности", "descr": "Температура поверхности, в отличие от приземной температуры воздуха, определяет степень нагретости поверхности земли или водоема"},
		"TMP_850_mb": {"steps": {"set": [1,3,6,12], "def": 6}, "name": "Температура на 850 гПа", "descr": "Температура на уровне 850 гПа, или примерно 1,5 км, используется обычно для оценки притоков и оттоков тепла в нижней тропосфере"},
		"TMP_700_mb": {"steps": {"set": [1,3,6,12], "def": 6}, "name": "Температура на 700 гПа", "descr": "Температура на уровне 700 гПа, или примерно 3 км, используется обычно для оценки притоков и оттоков тепла в нижней тропосфере"},
		"TMP_500_mb": {"steps": {"set": [1,3,6,12], "def": 6}, "name": "Температура на 500 гПа", "descr": "Температура на уровне 500 гПа, или примерно 5,5 км, используется обычно для оценки притоков и оттоков тепла в средней тропосфере"},
		"PRMSL_mean_sea_level": {"group": "popular", "steps": {"set": [1,3,6,12], "def": 6}, "name": "Давление на уровне моря", "descr": "Давление воздуха, приведенное к уровню моря. Карта обычно используется для оценки расположения циклонов и антициклонов в нижних слоях атмосферы"},
		"PRES_surface": {"steps": {"set": [1,3,6,12], "def": 6}, "name": "Давление на поверхности", "descr": "Карта показывает распределение давления на поверхности с учетом рельефа"},
		...
		"TSOIL_0-0.1_m_below_ground": {"steps": {"set": [1,3,6,12], "def": 6}, "name": "Температура почвы 0-10 см", "descr": "Температура почвы в верхнем слое представляет собой среднее значение в слое от поверхности до глубини 10 см. Этот параметр важен для оценки состояния травянистых сельскохозяйственных культур и плодовых кустов"},
		"TSOIL_0.1-0.4_m_below_ground": {"steps": {"set": [1,3,6,12], "def": 6}, "name": "Температура почвы 10-40 см", "descr": "Температура почвы в верхнем слое представляет собой среднее значение в слое на глубине от 10 до 40 см. Этот параметр важен для оценки состояния плодовых деревьев"},
		"TSOIL_0.4-1_m_below_ground": {"steps": {"set": [1,3,6,12], "def": 6}, "name": "Температура почвы 40-100 см", "descr": "Температура почвы в верхнем слое представляет собой среднее значение в слое на глубине от 40 до 100 см. Этот параметр важен для оценки состояния плодовых деревьев"},
		"TSOIL_1-2_m_below_ground": {"steps": {"set": [1,3,6,12], "def": 6}, "name": "Температура почвы 1-2 м", "descr": "Температура почвы в верхнем слое представляет собой среднее значение в слое на глубине от 1 до 2 метров. Этот параметр важен для оценки зимнего промерзания почвы"},
		"SOILW_0-0.1_m_below_ground": {"steps": {"set": [1,3,6,12], "def": 6}, "name": "Влажность почвы 0-10 см", "descr": "Влагосодержание почвы в верхнем слое до глубины 10 см. Влагосодержание почвы в верхнем слое представляет собой среднее долю влаги, содержащуюся в почве относительно ее максимальной влагоемкости. Этот параметр важен для оценки состояния травянистых сельскохозяйственных культур и плодовых кустов"},
		"SOILW_0.1-0.4_m_below_ground": {"steps": {"set": [1,3,6,12], "def": 6}, "name": "Влажность почвы 10-40 см", "descr": "Влагосодержание почвы в слое 10-40 см. Влагосодержание почвы в верхнем слое представляет собой среднее долю влаги, содержащуюся в почве относительно ее максимальной влагоемкости. Этот параметр важен для оценки состояния плодовых кустов и деревьев"},
		"HLCY_3000-0_m_above_ground": {"group": "new", "steps": {"set": [1,3,6,12], "def": 6}, "name": "Штормовая опасность", "descr": "Эта экспериментальная карта представляет оценку вероятности развитя сильных внутримассовых и прифронтальных конвективных систем и связанной с ними штормовой погоды (порывы ветра, гроза, град, смерч)"},
		"FORMULA_FIRE": {"group": "new", "steps": {"set": [1,3,6,12], "def": 6}, "name": "Класс пожароопасности", "descr": "Эта экспериментальная карта представляет оценку пожароопасности, рассчитанную на базе влажности верхнего слоя почвы. Данный метод является неофициальным и не совпадает с методикой оценки пожароопасности, принятой Росгидрометом"},
		"FORMULA_SNOWDENS": {"group": "new", "steps": {"set": [1,3,6,12], "def": 6}, "name": "Плотность снега", "descr": ""},
		"SNOD_surface": {"group": "new", "steps": {"set": [1,3,6,12], "def": 6}, "name": "Высота снега", "descr": ""}				
	};
  ...
  $(document).ready(function() {
	...
	var _lat = parseFloat(getParameterByName('fi')) || (typeof lat != 'undefined'?lat:<%= lat %>),
	    ...
	    timeline: new Timeline(
	        $('.timeline'),
	        {summertime: 0, timeshift: -999, predict: 6, step: step, count: 24, maptype: type, display: 'block', select_hours: $('select[name="hours"]')} 
	    ),
